...
const branchInfo: Record<UpdateBranch, { label: string; description: string; badgeClass: string }> = {
    main: { label: 'Main', description: 'Stabil - Empfohlen fuer Produktivumgebungen', badgeClass: 'badge-success' },
    dev: { label: 'Dev', description: 'Entwicklung - Neue Features, moeglicherweise instabil', badgeClass: 'badge-info' },
...
export default function UpdateManager() {
    ...
    const saveBranch = async () => {
        ...
        if (isUpgrade) {
            const ok = await modal.confirm({
                ...
                message: 'Der Experimental-Branch enthaelt die neusten Commits und ist moeglicherweise instabil. Fehler und Datenverlust sind moeglich.\n\nSind Sie sicher?',
            ...
            const ok = await modal.confirm({
                ...
                message: 'Achtung: Ein Wechsel auf einen aelteren Branch kann zu Inkompatibilitaeten fuehren. Stellen Sie sicher, dass Sie ein Backup haben.\n\nFortfahren?',
                confirmText: 'Wechseln',
                variant: 'warning',
            });
            if (!ok) return;
    ...
    const saveInterval = async () => {
        ...
        try {
            ...
            if (res.ok) {
                toast.success('Pruef-Intervall gespeichert');
            } else {
                const data = await res.json().catch(() => ({}));
    ...
    if (loading) return <div className="text-muted">Pruefe auf Updates...</div>;
    ...
    // Neuestes Backup pruefen fuer Rollback-Info
    ...
    const intervalOptions = [
        { value: 10, label: '10 Sekunden (Test)' },
    ...
    return (
        <div>
            ...
            <div className="card mb-md">
                    ...
                    <div style={{ marginTop: 'var(--space-md)', display: 'flex', alignItems: 'center', gap: 'var(--space-md)' }}>
                        ...
                        {selectedBranch === 'experimental' && (
                            <span style={{ color: 'var(--color-warning)', fontSize: 'var(--font-size-sm)', display: 'flex', alignItems: 'center', gap: 4 }}>
                                {warningIcon} Instabil - Nur fuer Entwicklung empfohlen
                            </span>
                        )}
                    </div>
                )}
                ...
                <div style={{ marginTop: 'var(--space-lg)', paddingTop: 'var(--space-md)', borderTop: '1px solid var(--color-border)' }}>
                    ...
                    <div className="text-muted" style={{ fontSize: 'var(--font-size-xs)', marginTop: 4 }}>
                        Wie oft automatisch nach Updates gesucht wird. Bei verfuegbarem Update erhalten Admins eine Benachrichtigung.
                    </div>
                </div>
            </div>
            ...
            {core && (
                <div className="card mb-md">
                    <div className="flex-between mb-md">
                        ...
                        {core.available ? (
                            <span className="badge badge-success">Update verfuegbar</span>
                        ) : (
            ...
            {isRecentBackup && newestBackup && (
                <div className="card mb-md" style={{ borderColor: 'var(--color-warning)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 'var(--space-sm)' }}>
                        {warningIcon}
                        <strong style={{ fontSize: 'var(--font-size-sm)' }}>Rollback verfuegbar</strong>
                    </div>
            ...
            <div className="card mb-md">
                <div className="flex-between">
                    <div className="card-title">Changelog-Verlauf</div>
                    <button
                        className="btn btn-secondary btn-sm"
                        onClick={() => {
                            if (!showChangelog && !changelogHistory) {
                ...
                {showChangelog && changelogHistory && (
                    <div style={{ marginTop: 'var(--space-md)', maxHeight: 400, overflowY: 'auto' }}>
                        {changelogHistory.releases && changelogHistory.releases.length > 0 && (
                            changelogHistory.releases.map((release, idx) => (
                                <div key={idx} style={{
                                    padding: 'var(--space-md)',
                                    borderBottom: idx < changelogHistory.releases!.length - 1 ? '1px solid var(--color-border)' : 'none',
                                }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                        <strong>v{release.version}</strong>
                                        {release.name && <span className="text-muted" style={{ fontSize: 'var(--font-size-sm)' }}>- {release.name}</span>}
                                        {release.prerelease && <span className="badge badge-info" style={{ fontSize: 'var(--font-size-xs)' }}>Pre-Release</span>}
                                    </div>
                                    <div className="text-muted" style={{ fontSize: 'var(--font-size-xs)', marginTop: 2 }}>
                                        {formatDate(release.publishedAt)}
                                    </div>
                                    {release.body && (
                                        <div style={{ marginTop: 'var(--space-sm)', fontSize: 'var(--font-size-sm)', whiteSpace: 'pre-wrap', lineHeight: 1.5 }}>
                                            {release.body}
                                        </div>
                                    )}
                                </div>
                        ...
                        {changelogHistory.commits && changelogHistory.commits.length > 0 && (
                            changelogHistory.commits.map((commit, idx) => (
                                <div key={idx} style={{
                                    padding: '6px 0',
                                    borderBottom: idx < changelogHistory.commits!.length - 1 ? '1px solid var(--color-border)' : 'none',
                                    display: 'flex',
                                    gap: 8,
                                    fontSize: 'var(--font-size-sm)',
                                }}>
                                    <code style={{ fontSize: 'var(--font-size-xs)', color: 'var(--color-primary)', flexShrink: 0 }}>{commit.hash}</code>
                                    <span style={{ flex: 1 }}>{commit.message}</span>
                                    <span className="text-muted" style={{ fontSize: 'var(--font-size-xs)', flexShrink: 0 }}>{formatDate(commit.date)}</span>
                                </div>
                            ))
                        )}

                        {!changelogHistory.releases?.length && !changelogHistory.commits?.length && (
                            <p className="text-muted">Keine Eintraege gefunden.</p>
                        )}
                    </div>
                )}
            </div>
            ...
            <div className="card mb-md">
                ...
                {backups.length === 0 ? (
                    ...
                    <div className="table-container">
                        <table>
                            <thead>
                                <tr><th>Datum</th><th>Branch</th><th>Groesse</th><th>Restore-Befehl</th></tr>
                            </thead>
                            <tbody>
                                {backups.map((backup) => (
            ...
            <div className="card mb-md">
                ...
                {installedPluginsSorted.length === 0 ? (
                    ...
                    <div className="table-container">
                        <table>
                            <thead>
                                <tr><th>Plugin</th><th>Name</th><th>Status</th><th>Installiert</th><th>Verfuegbar</th><th>Aktionen</th></tr>
                            </thead>
                            <tbody>
            ...
            {/* 7. Verfuegbare Plugins */}
            <div className="card">
                <div className="card-title mb-md">Verfuegbare Plugins</div>
                {installableRemotePlugins.length === 0 ? (
                    <p className="text-muted">Keine weiteren Plugins verfuegbar.</p>
                ) : (
