...
export default async function adminRoutes(fastify: FastifyInstance): Promise<void> {
    ...
    // Alle Admin-Routen brauchen Auth
    ...
    fastify.get('/audit-log', { preHandler: [requirePermission('audit.view')] }, async (request, reply) => {
        const { category, userId, pluginId, action, from, to, tenantId, page = 1, limit = 50 } = request.query as any;
    ...
    fastify.get('/updates/check', { preHandler: [requirePermission('updates.manage')] }, async (request, reply) => {
        const { checkCoreUpdate, checkPluginUpdates, getRemoteCatalog, getInstalledPlugins } = await import('../services/updater.js');

        const [coreUpdate, pluginUpdates, catalog, installedPlugins] = await Promise.all([
            checkCoreUpdate(),
            checkPluginUpdates(),
            getRemoteCatalog(),
            getInstalledPlugins(),
        ...
        return reply.send({ core: coreUpdate, plugins: pluginUpdates, catalog, installedPlugins });
    });
    ...
    // POST /api/admin/updates/install-core
    fastify.post('/updates/install-core', { preHandler: [requirePermission('updates.manage')] }, async (request, reply) => {
        const { installCoreUpdate } = await import('../services/updater.js');
        const task = createUpdateTask('core', 'core');

        await fastify.audit.log({
            action: 'admin.update.core_install',
            category: 'admin',
        }, request);

        runUpdateTask(task.id, async (reporter) => {
            const result = await installCoreUpdate({
                onProgress: ({ message, progress }) => reporter.log(message, progress),
            });

            if (!result.success) {
                reporter.fail(result.error || 'Core-Update fehlgeschlagen');
                return;
            }

            reporter.success(`Core-Update auf v${result.version || '-'} erfolgreich`, result.version);
        });
