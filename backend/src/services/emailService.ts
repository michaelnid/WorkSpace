...
export async function emailRoutes(fastify: FastifyInstance): Promise<void> {
    ...
    fastify.get('/email/settings', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const settings = await getEmailSettings();
        // Passwort maskieren
    ...
    fastify.put('/email/settings', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    fastify.post('/email/test', { preHandler: [fastify.authenticate] }, async (request, reply) => {
